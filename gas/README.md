...
260727에 확인했다. 답변KB 시트가 바뀌면 라인브릿지가 **미러를 두 개 더** 만든다.
슬랙 #회의_프로그램에 `🔄 KB 미러 동기화 — 답변KB N건 반영`으로 찍힌다.

| 미러 | 위치 | 파일 |
|---|---|---|
| 깃허브 | `acro_archive` 리포 | `KB_QA_standard_latest.md` · `qa374_latest.json` |
| 구글드라이브 | `04_팀클로드_KB` | 같은 두 파일 |

그래서 빌드 한 번이 **연쇄로 4곳**을 건드린다. 빌드가 418행을 한꺼번에 바꾸면 이 동기화도
그만큼 돈다. 260727 슬랙 폭주의 유력한 원인이다.

→ 미러 동기화가 **변경 건수에 비례해 돌지 않도록**(한 번에 한 번만, 요약 한 줄만) 조절해야 한다.
그 코드는 라인브릿지 `Code.js`에 있고 아직 손대지 않았다.
