...
## 야간 드리프트 감사 (M3 · audit_kb.gs)

같은 프로젝트에 `audit_kb.gs`를 함께 넣으면 야간 감사가 생긴다.
설치는 한 번만 `acdAuditInstall()` 실행 — 매일 03시대 트리거가 걸린다.

매일 밤 **읽기 전용**으로 3자 대조한다: 정본(Drive md) ↔ 답변KB 시트 ↔ qa374.json(공개 리포).

보고 정책 — 260727 슬랙 폭주의 교훈을 그대로 설계에 넣었다.

| 상황 | 행동 |
|---|---|
| 전부 일치 | **침묵** |
| 7일 연속 정상 | 생존 신호 1줄 (센서가 죽어 조용한 것과 구분하기 위해) |
| 어긋남 | 1메시지, 최대 8줄 + "외 N건" |
| 공개 리포에 민감정보 | 🚨 최우선 경보 (계좌·전화·주민번호 패턴, 오탐 방지 숫자 10자리 규칙 포함) |
| 감사 자체가 죽음 | 1회만 알림, 같은 오류로 매일 울리지 않음 |

검증: `node gas/test/test_audit.js <build_kb.gs> <audit_kb.gs> <정본.md> <시트스냅샷.json> <qa374.json>`
실데이터 회귀 3종 — 실제 드리프트 감지(재탕전·보관 판정 수정분을 잡아냄) · 깨끗하면 침묵 · 유출 경보.
