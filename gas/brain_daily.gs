 ...
 * 시트·정본·환자 대면 채널에 쓰지 않는다.
 * ⚠ 게시 대상은 **원내 직원 채널**이다 — 환자 실명·차트번호를 그대로 쓴다(원장 지시 260820).
 *   arnMask_는 전화·이메일만 가린다(이름은 건드리지 않는다). 이름 축약은 업무 방해다.
 */
...
function acbTaskPrompt_() {
  return [
    ...
    '아래는 아크로한의원 슬랙의 오늘 메시지 다이제스트다(전화·이메일만 가려져 있고, 이름은 그대로다).',
    '',
    ...
    '',
    '**이름은 그대로 쓴다.** 이 채널은 원내 직원 전용이고, 직원들은 이미 실명으로 대화한다.',
    '"H님"·"I님" 같은 이니셜 축약은 누구 얘기인지 알 수 없게 만들어 업무를 방해한다 —',
    '다이제스트에 이름이 보이면 그 이름을 그대로 쓰고, 차트번호가 보이면 그것도 함께 쓴다.',
    '(원장 지시 260820: "직원끼리 보는데 왜 마스킹하나. 업무 능률만 떨어뜨린다.")',
    '',
    '금지: 추측으로 채우기(확인 안 되면 "확인 불가"), 영업성 문구, 다이제스트에 없는 내용.'
  ].join('\n');
}
