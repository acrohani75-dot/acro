...
const parsed = sandbox.acdParseCanon_(text);
...
console.log('\n== 이모지 정규화 ==');
const norm = sandbox.acdNormalizeAll_(parsed.items);
ok(norm.changed === 0, 'v1.6 정본에 남은 숏코드 0곳', '치환 ' + norm.changed);
ok(norm.leftover.length === 0, '매핑 밖 숏코드 0종', norm.leftover.join(','));

console.log('\n== json 산출물 ==');
const res = sandbox.acdWriteJson_(parsed.items, true);
console.log('  ' + res.msg);

console.log('\n== 시트 열 매핑 ==');
const first = parsed.items[0];
const row = sandbox.ACD_CFG.SHEET_COLS.map(col => {
  for (const f in sandbox.ACD_CFG.FIELD_MAP) if (sandbox.ACD_CFG.FIELD_MAP[f] === col) return first[f] || '';
  return '';
});
