...
ok('가능/불가 단정 없음 + 직원 확정 문구', !d.includes('불가 —') && d.includes('확정 처리는 직원이'));
ok('답장 문구 없음 — 빈 시각 제안만(원장 판정: 채팅 답장 아님)',
   !d.includes('답장') && !d.includes('안녕하세요') && d.includes('제안: 8/15 오후 1:00'));
d=sandbox.aciDraft_(p,{'2026-08-15':null});
ok('조회 실패는 "확인 불가"(없음이 아님)', d.includes('확인 불가') && !d.includes('기존 예약 없음'));
ok('전부 확인 불가면 제안 없이 직원 판단', d.includes('직원 판단') && !d.includes('제안:'));
d=sandbox.aciDraft_(p,{'2026-08-15':['13:00','13:30','14:00']});
ok('전부 차 있으면 제안 없이 직원 판단', d.includes('직원 판단') && !d.includes('제안:'));
