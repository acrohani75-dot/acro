 ...
 *
 * ─── 실행 순서 ────────────────────────────────────────────────
 *   1) buildDryRun()   아무것도 쓰지 않는다. 리포트만 낸다. **먼저 이걸 통과시킨다.**
 *   2) buildAll()      실제 반영.
 *
 * ─── 절대 규칙 ────────────────────────────────────────────────
...
function buildDryRun() { return run_(true); }
function buildAll() { return run_(false); }
...
function run_(dry) {
  ...
  try {
    ...
    var parsed = parseCanon_(chk.text);
    log.push('파싱 ' + parsed.items.length + '건');
    if (parsed.warnings.length) {
      log.push('⚠ 파싱 경고 ' + parsed.warnings.length + '건: ' + parsed.warnings.slice(0, 5).join(' / '));
    }

    if (parsed.items.length < CFG.MIN_ITEMS) {
      throw new Error('정본이 ' + parsed.items.length + '건뿐이다(최소 ' + CFG.MIN_ITEMS +
    ...
    var norm = normalizeAll_(parsed.items);
    if (norm.changed) log.push('이모지 숏코드 정규화 ' + norm.changed + '곳');

    var sheetRes = writeSheet_(parsed.items, dry);
    log.push(sheetRes.msg);

    var jsonRes = writeJson_(parsed.items, dry);
...
function selfCheck_() {
  ...
  var it = DriveApp.getFoldersByName(CFG.CANON_FOLDER);
  if (!it.hasNext()) {
    throw new Error('Drive 폴더 `' + CFG.CANON_FOLDER + '`를 읽을 수 없다. ' +
  ...
  if (!ss.getSheetByName(CFG.SHEET_TAB)) throw new Error('시트 탭 `' + CFG.SHEET_TAB + '`이 없다.');
...
function parseCanon_(text) {
  ...
  for (var b = 0; b < blocks.length; b++) {
    ...
    for (var i = 1; i < lines.length; i++) {
      ...
      if (m) {
        if (cur !== null) { obj[cur] = trimEnd_(buf.join('\n')); buf = []; cur = null; }
...
function normalizeAll_(items) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    for (var k in it) {
      ...
      var v = it[k], before = v;
      for (var code in CFG.EMOJI) {
        while (v.indexOf(code) >= 0) { v = v.replace(code, CFG.EMOJI[code]); changed++; }
      }
      if (v !== before) it[k] = v;
  ...
  if (keys.length) {
    slack_('⚠ 매핑에 없는 이모지 숏코드가 정본에 있다 — `' + keys.join('`, `') +
      '`\n`CFG.EMOJI`에 추가하지 않으면 환자 화면에 그대로 찍힌다.');
  }
...
function writeSheet_(items, dry) {
  var props = PropertiesService.getScriptProperties();
  var sh = SpreadsheetApp.openById(props.getProperty('KB_SPREADSHEET_ID')).getSheetByName(CFG.SHEET_TAB);
  ...
  var rows = items.map(function (it) {
    var id = it['원본ID'];
    canonIds[id] = true;
    return CFG.SHEET_COLS.map(function (col) {
      for (var f in CFG.FIELD_MAP) if (CFG.FIELD_MAP[f] === col) return it[f] || '';
      return '';
    });
  });
  ...
  for (var r = 1; r < old.length; r++) {
    ...
    if (!id) continue;
    if (!canonIds[id]) { sheetOnly.push(id); keep.push(old[r].slice(0, CFG.SHEET_COLS.length)); }
  }

  var total = rows.length + keep.length;
  var oldCount = Math.max(0, old.length - 1);
  if (oldCount - total > CFG.MAX_SHRINK) {
    throw new Error('시트 행이 ' + oldCount + ' → ' + total + '로 ' + (oldCount - total) +
      '건 줄어든다(허용 ' + CFG.MAX_SHRINK + '). 정본 누락이 의심되어 중단한다.');
  }
  ...
  var out = [CFG.SHEET_COLS].concat(rows).concat(keep);
  sh.clearContents();
  sh.getRange(1, 1, out.length, CFG.SHEET_COLS.length).setValues(out);
...
function writeJson_(items, dry) {
  var payload = {
    ...
    source: '정본 L2_응대KB (Drive ' + CFG.CANON_FOLDER + ') 에서 자동 빌드. 이 파일을 직접 고치지 말 것 — 다음 빌드에서 덮인다.',
    count: items.length,
    items: items.map(function (it) {
      var o = {};
      CFG.SHEET_COLS.forEach(function (col) {
        for (var f in CFG.FIELD_MAP) if (CFG.FIELD_MAP[f] === col) o[col] = it[f] || '';
      });
      CFG.LANGS.forEach(function (lg) {
