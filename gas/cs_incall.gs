...
function aciDraft_(p, bookedByDate) {
  ...
  if (firstFree) {
    lines.push('');
    lines.push('답장 초안(복붙용): 안녕하세요, 아크로한의원입니다 😊 문의해주셔서 감사합니다. '
      + '희망하신 ' + firstFree.raw + ' 시간으로 안내 도와드리겠습니다. 방문 전 확인차 연락드릴게요!');
  } else {
    lines.push('');
    lines.push('※ 희망 시각 전부 기존 예약이 있거나 확인 불가 — 직원 판단 후 답장 바랍니다.');
  }
  lines.push('(대조는 참고용 — 베드·원장 배정에 따라 같은 시각도 가능할 수 있음. 확정은 직원이.)');
  return lines.join('\n');
}
