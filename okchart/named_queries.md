...
- ⚠ **201PC에는 SQL 인스턴스가 2개다** (원장 확정 260830).
  `MSSQL$OKCHART` = 우리 것. `MSSQL$IEMBIO` = **별개 시스템, 건드리지 않는다.**
  워커 접속 문자열이 인스턴스를 명시하는지 확인할 것 — 서버명만 적으면 어느 쪽에 붙을지 보장이 없고,
  남의 DB에 붙은 조회는 조용히 빈 결과를 돌려줘 "데이터가 없다"로 오독된다.
- **조회가 전부 실패할 때 — 201 SQL 로그인이 벤더 업데이트로 꺼졌을 수 있다** (260830).
  이제 **진료실PC에서 원격으로 되살릴 수 있다** — 전용 관리 로그인(`ALTER ANY LOGIN`)이 있어
  단일 사용자 모드 재시작이 필요 없다. 계정명·절차는 Drive 운영문서 **`PC구성및호칭_운영문서`**
  (title 검색, 최신본) — 공개 리포라 여기 적지 않는다.
  ⚠ 이건 **SQL 로그인이 꺼진 경우**의 복구다. 워커 프로세스가 죽은 것과는 다른 고장이고,
  201PC 기본 Claude 창은 일반권한이라 **서비스 중지·재시작은 여전히 그 창으로 못 한다.**
